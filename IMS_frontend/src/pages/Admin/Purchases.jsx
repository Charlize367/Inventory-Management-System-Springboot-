...
const Purchases = () => {
  ...
  const closeModal2 = () => setShowModal2(false);
    ...
    const headers = [
    { key: 'purchaseId', header: 'ID' },
    { key: 'purchaseItems', header: 'Items' },
    { key: 'variationOptions', header: 'Variation'},
    { key: 'supplier', header: 'Supplier' },
    { key: 'staff', header: 'Staff' },
    { key: 'purchaseAmount', header: 'Amount' },
    { key: 'purchaseDate', header: 'Date' },
    { key: 'purchaseStatus', header: 'Status' },
    ...
    {
        Header: 'Items',
        accessor: 'purchaseItems.product.productName',
    },
    {
      Header: 'Variation',
      accessor: 'purchaseItems.variationOptions.variationOptionName'
    },
    {
        Header: 'Supplier',
    ...
    const handleChange = (e) => {
        ...
        setFormData({ ...formData, [name]: name === "supplierId" ? Number(value) : value,
            });
    }
  ...
  setSelectedItems(prevItems =>
    ...
    const getPurchases = async (page = 0) => {

      let endpoint = ``;
      if(role === "ADMIN" || role === "MANAGER") {
        endpoint = `purchases?page=${page}&size=${pageSize}`;
      } else if (role === "STAFF") {
        endpoint =`purchases?staffId=${userId}&page=${page}&size=${pageSize}`
      }
    ...
    return;
       ...
       try {
        ...
        if (isEditing) {
            ...
            const response = await axios.put(`${API_URL}/purchases/${formData.purchaseId}`, formData, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
                },  
  ...
  <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 lg:gap-0 w-full m-0 p-0 overflow-x-hidden min-h-screen">
      ...
      <div className="bg-white m-0 p-0 flex flex-col">
        ...
        <div className="hidden lg:flex items-center justify-between p-4 border-b border-gray-300">
          <h1 className="text-2xl block rounded-lg px-4 py-2 font-medium text-gray-500">Purchases</h1>
        </div>
        ...
        <tbody>
           {selectedItems.map((row, rowIndex) =>  {
            ...
            return(
          <tr className="bg-white border-b border-gray-200 text-gray-500 hover:bg-gray-100 ">
      ...
      prev.map((item, i) =>
        i === rowIndex
          ...
          : item
                                    ...
                                    <option value="" className="text-gray-50 w-10">Select product</option>
                                    {products.map(p => (
                                        <option key={p.productId} value={p.productId}>{p.productName}</option>
                                    ))}
                                </select></td>
                    <td className="px-6 py-4">{product?.productCost || 0 }</td>
