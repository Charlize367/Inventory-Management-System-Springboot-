...
const Manager = () => {
  ...
  const handlePopup = (message) => {
    ...
    const getManager = async (page = 0) => {
       try {
             const response = await axios.get(`${API_URL}/users?page=${page}&size=${pageSize}&role=manager`, {
                  headers: {
              ...
              setManager(response.data.content);
...
console.log(manager);
    ...
    return;
       ...
       try {
        ...
        if (isEditing) {
            ...
            getManager();
            handlePopup("Manager edited successfully.");
            closeAndClearForm();
             ...
             const response = await axios.post(`${API_URL}/users`, formData,   {
              ...
              getManager();
              handlePopup("Manager added successfully.");
              closeAndClearForm();
    ...
    const deleteManager = async(id) => {
        ...
        msg = "Cannot delete this manager because they are linked to other records.";
  ...
  <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 lg:gap-0 w-full m-0 p-0 overflow-x-hidden min-h-screen">
        ...
        <div className="flex items-center justify-between bg-white p-4 lg:hidden">
          <h1 className="text-2xl block rounded-lg bg-white px-4 py-2 font-medium text-gray-700">Managers</h1>
          <button
      ...
      <div className="bg-white m-0 p-0 flex flex-col">
        ...
        <div className="hidden lg:flex items-center justify-between p-4 border-b border-gray-300">
          <h1 className="text-2xl block rounded-lg px-4 py-2 font-medium text-gray-500">Managers</h1>
        </div>

       
        <div className="grid grid-cols-1 gap-4 m-0  bg-gray-200 p-4 flex-grow">
          <Table headers={headers} columns={columns} onAddClick={openModal} onEditClick={handleEdit} disableAdd="false" disableEdit="false" data={manager} onDeleteClick={handleDelete} currentPage={currentPage} totalPages={totalPages} fetchData={getManager} />
        ...
        {showPopup && (
                ...
                <div className="bg-white p-6 rounded shadow-lg w-80 text-center">
                    ...
                    <button
                    onClick={() => { setShowPopup(false); getManager(currentPage); }}
        ...
        {showModal2 && (<DeleteConfirm onClose={closeModal2} onDeleteClick={() => deleteManager(deleteId)} element="manager" />)}
        ...
        {deleteSuccess && (
            <DeleteSuccessPopup element="Manager" />
        )}
        </div>
