...
const Brands = () => {
    ...
    const getBrands = async(page = 0) => {
       try {
             const response = await axios.get(`${API_URL}/brands?page=${page}&size=${pageSize}`, {
                  headers: {
    ...
    console.log(formData);
       ...
       try {
        if (isEditing) {
            ...
            const response = await axios.put(`${API_URL}/brands/${formData.brandId}`, formData, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
                },  
            ...
            getBrands();
            handlePopup("Brand edited successfully.");
            closeAndClearForm();
             ...
             const response = await axios.post(`${API_URL}/brands`, formData,   {
              ...
              getBrands();
              handlePopup("Brand added successfully.");
              closeAndClearForm();
    ...
    const deleteBrand = async(id) => {
        ...
        msg = "Cannot delete this brand because they are linked to other records.";
...
console.log(deleteId);
  return (
        ...
        <div className="flex items-center justify-between bg-white p-4 lg:hidden">
          <h1 className="text-2xl block rounded-lg bg-white px-4 py-2 font-medium text-gray-700">Brands</h1>
          <button
      ...
      <div className="bg-white m-0 p-0 flex flex-col">
        ...
        <div className="hidden lg:flex items-center justify-between p-4 border-b border-gray-300">
          <h1 className="text-2xl block rounded-lg px-4 py-2 font-medium text-gray-500">Brands</h1>
        </div>

        
        <div className="grid grid-cols-1 gap-4 m-0  bg-gray-200 p-4 flex-grow">
          <Table headers={headers} columns={columns} onAddClick={openModal} onEditClick={handleEdit} disableAdd="false" disableEdit="false" data={brands} onDeleteClick={handleDelete} currentPage={currentPage} totalPages={totalPages} fetchData={getBrands}  />
        ...
        {showPopup && (
                ...
                <div className="bg-white p-6 rounded shadow-lg w-80 text-center">
                    ...
                    <button
                    onClick={() => { setShowPopup(false); getBrands(currentPage); }}
    ...
    {showModal && (
                    ...
                    <form onSubmit={handleSubmit} className="p-4 md:p-5">
                        <div className="grid gap-4 mb-4 grid-cols-2">
                            <div className="col-span-2">
                                <label htmlFor="brandName" className="block mb-2 text-sm font-medium text-gray-900">Name</label>
                                <input type="text" name="brandName" id="brandName" value={formData.brandName} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Brand" required/>
                            </div>
        ...
        {showModal2 && (<DeleteConfirm onClose={closeModal2} onDeleteClick={() => deleteBrand(deleteId)} element="brand" />)}
        ...
        {deleteSuccess && (
            <DeleteSuccessPopup element="Brand" setDeleteSuccess={setDeleteSuccess} />
        )}
