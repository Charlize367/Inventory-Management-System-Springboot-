...
const StockMovements = () => {
  ...
  const [stockMovements, setStockMovement] = useState([]);
  ...
  const handlePopup = (message) => {
    ...
    const headers = [
    { key: 'stockMovementId', header: 'ID' },
    { key: 'productId', header: 'Product' },
    { key: 'quantityChange', header: 'Quantity' },
    { key: 'stockMovement', header: 'Type' },
    ...
    const handleEdit = async (stockMovement) => {
        setFormData({
            stockMovementId: stockMovement.stockMovementId,
            productId: stockMovement.product.productName,
    ...
    const getStockMovement = async (page = 0) => {
       try {
             const response = await axios.get(`${API_URL}/stockMovements?page=${page}&size=${pageSize}`, {
                  headers: {
                       'Content-Type': 'application/json',
                       'Authorization': `Bearer ${token}`
                  }
              ...
              setStockMovement(response.data.content);
    ...
    console.log(formData);
            ...
            const response = await axios.put(`${API_URL}/stockMovements/${formData.id}`, formData, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
                },  
            ...
            getStockMovement();
            handlePopup("Stock movement edited successfully.");
            closeAndClearForm();
    ...
    const deleteStockMovements = async(id) => {
        try {
          const response = await axios.delete(`${API_URL}/stockMovements/${id}`, {
              headers: {
                        'Authorization' : `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    }});
  ...
  <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 lg:gap-0 w-full m-0 p-0 overflow-x-hidden min-h-screen">
        ...
        <div className="flex items-center justify-between bg-white p-4 lg:hidden">
          <h1 className="text-2xl block rounded-lg bg-white px-4 py-2 font-medium text-gray-700">Stock Movements</h1>
          <button
      ...
      <div className="bg-white m-0 p-0 flex flex-col">
        ...
        <div className="hidden lg:flex items-center justify-between p-4 border-b border-gray-300">
          <h1 className="text-2xl block rounded-lg px-4 py-2 font-medium text-gray-500">Stock Movements</h1>
        </div>

       
        <div className="grid grid-cols-1 gap-4 m-0  bg-gray-200 p-4 flex-grow">
          <Table headers={headers} columns={columns} onEditClick={handleEdit} disableEdit="false" disableAdd="true" data={stockMovements} onDeleteClick={handleDelete} currentPage={currentPage} totalPages={totalPages} fetchData={getStockMovement}/>
        ...
        {showPopup && (
                ...
                <div className="bg-white p-6 rounded shadow-lg w-80 text-center">
                    ...
                    <button
                    onClick={() => { setShowPopup(false); getStockMovement(currentPage); }}
            ...
            <div className="relative p-4 w-full max-w-md max-h-full">
                ...
                <div className="relative bg-gray-100 rounded-lg shadow-sm ">
                    ...
                    <div className="flex items-center justify-between p-4 md:p-5 rounded-t dark:border-gray-600 border-gray-100">
                        ...
                        <button type="button" onClick={closeModal} className="text-gray-400 cursor-pointer bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="crud-modal">
                    ...
                    <form className="p-4 md:p-5" onSubmit={handleSubmit}>
                        <div className="grid gap-4 mb-4 grid-cols-2">
                             <div className="col-span-2">
                                <label htmlFor="price" className="block mb-2 text-sm font-medium text-gray-900">Stock </label>
                                <input type="number" value={formData.quantityChange} onChange={handleChange} name="productStock" id="productStock" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Stocks" required/>
                            </div>
        ...
        {showModal2 && (<DeleteConfirm onClose={closeModal2} onDeleteClick={() => deleteStockMovements(deleteId)} element="stock movement record" />)}
        ...
        {deleteSuccess && (
            <DeleteSuccessPopup element="Stock Movement" />
        )}
        </div>
